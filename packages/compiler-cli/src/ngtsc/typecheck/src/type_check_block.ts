...
/**
 * Creates a variable declaration of the argument type and adds it to the tcb. Returns the id of the
 * newly created variable.
 */
function createVariableDeclarationOfType(
    tcb: Context, node: TmplAstTemplate|TmplAstElement, scope: Scope,
    type: ts.TypeNode): ts.Identifier {
  const id = tcb.allocateId();
  addExpressionIdentifier(type, ExpressionIdentifier.DIRECTIVE);
  addParseSpanInfo(type, node.startSourceSpan || node.sourceSpan);
  scope.addStatement(tsDeclareVariable(id, type));
  return id;
}
...
class TcbDirectiveTypeOp extends TcbOp {
  ...
  execute(): ts.Identifier {
    ...
    return createVariableDeclarationOfType(this.tcb, this.node, this.scope, type);
  }
}
 ...
 */
class TcbGenericDirectiveTypeWithAnyParamsOp extends TcbOp {
  constructor(
      private tcb: Context, private scope: Scope, private node: TmplAstTemplate|TmplAstElement,
      private dir: TypeCheckableDirectiveMeta) {
    super();
  ...
  execute(): ts.Identifier {
    ...
    if (dirRef.node.typeParameters === undefined) {
      throw new Error(
          `Expected typeParameters when creating a declaration for ${dirRef.debugName}`);
    }

    const typeArguments = dirRef.node.typeParameters.map(
        () => ts.factory.createKeywordTypeNode(ts.SyntaxKind.AnyKeyword));
    const type = ts.factory.createTypeReferenceNode(rawType.typeName, typeArguments);
